...
async function removeCheatFromSavegame() {
  return new Promise((resolve, reject) => {
    ...
    for (const file of files) {
      ...
      while(hexBuffer.indexOf(Buffer.from([0x04, 0x00, 0x00, 0x00, 0x01, 0x00, 0x001])) !== -1) {
          const offset = hexBuffer.indexOf(Buffer.from([0x04, 0x00, 0x00, 0x00, 0x01, 0x00, 0x01]))
          console.log(`[+] Removed cheat flag (command/cheat) from offset ${offset}`)
          hexBuffer[offset + 4] = 0x00
          changedCount++
      }
      ...
      while(hexBuffer.indexOf(Buffer.from([0x04, 0x00, 0x00, 0x00, 0x01, 0x00, 0x01])) !== -1) {
          const offset = hexBuffer.indexOf(Buffer.from([0x04, 0x00, 0x00, 0x00, 0x01, 0x00, 0x01]))
